...
#SBATCH --output=/projects/jgallucci/PALM/PALM_conmat_T_group_diff_Low_High_covar_%j.txt
#SBATCH --array=1-3
...
sublistids="${DIR}/sub_ids/Low_High.txt"
#assign full path for filename files, which is a list of path to GLM output in surface space
filename="$(find $DIR/file_ls/Low_High/* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/file_ls/PALM_group_diff_Low_High_con00/;s/.txt//')"
outdir="${DIR}/Results/$truncname"
desmat="${DIR}/con_design_matrix/groupdiff_Low_High_design_age_sex.csv"
...
surfL=${HCP_DATA}/RTMSWM/in_progress/ciftify/${exampleSubid}/MNINonLinear/fsaverage_LR32k/${exampleSubid}.L.midthickness.32k_fs_LR.surf.gii
surfR=${HCP_DATA}/RTMSWM/in_progress/ciftify/${exampleSubid}/MNINonLinear/fsaverage_LR32k/${exampleSubid}.R.midthickness.32k_fs_LR.surf.gii
