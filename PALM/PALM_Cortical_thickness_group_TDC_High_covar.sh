...
#SBATCH --output=/projects/jgallucci/PALM/PALM_conmat_simple_T_thickness_group_diff_Ctrl_High_Covar%j.txt
#SBATCH --array=1
...
filename="$(find $DIR/file_ls/Ctrl_High/thick.txt -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/thick/PALM_group_diff_Ctrl_High_cortical_thick/;s/.txt//')"
