...
#SBATCH --output=/projects/jgallucci/PALM/PALM_conmat_T_group_diff_Ctrl_Low_covar_%j.txt
#SBATCH --array=1-3
...
sublistids="${DIR}/sub_ids/Ctrl_Low.txt"
#assign full path for filename files, which is a list of path to GLM output in surface space
filename="$(find $DIR/file_ls/Ctrl_Low/* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/file_ls/PALM_group_diff_Ctrl_Low_con00/;s/.txt//')"
outdir="${DIR}/Results/$truncname"
desmat="${DIR}/con_design_matrix/groupdiff_Ctrl_Low_design_age_sex.csv"
...
meansurface() {
    ...
    while read subids; do
	  dir=/projects/jgallucci/VA_files/Ctrl_Low
