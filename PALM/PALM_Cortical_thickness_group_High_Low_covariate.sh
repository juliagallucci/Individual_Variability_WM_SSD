...
#SBATCH --output=/projects/jgallucci/PALM/PALM_conmat_simple_T_thickness_group_diff_High_Low_Covar%j.txt
#SBATCH --array=1
...
sublistids="${DIR}/sub_ids/High_Low.txt"
#assign full path for filename files, which is a list of path to GLM output in surface space
filename="$(find $DIR/file_ls/High_Low/thick.txt -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/thick/PALM_group_diff_High_Low_cortical_thick/;s/.txt//')"
outdir="${DIR}/Results/$truncname"
desmat="${DIR}/con_design_matrix/groupdiff_High_Low_design_age_sex.csv"
