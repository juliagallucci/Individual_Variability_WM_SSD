...
#SBATCH --output=/projects/jgallucci/PALM/PALM_conmat_simple_T_High_group_%j.txt
#SBATCH --array=1-3
...
sublistids="${DIR}/sub_ids/High_Var.txt"
#assign full path for filename files, which is a list of path to GLM output in surface space
filename="$(find $DIR/file_ls/High_Var/* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/file_ls/PALM_group_High_con00/;s/.txt//')"
...
meansurface() {
    ...
    while read subids; do
	  dir=/projects/jgallucci/VA_files/High_Var
