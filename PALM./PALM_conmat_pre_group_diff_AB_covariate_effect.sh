...
#SBATCH --output=/projects/jgallucci/PALM/PALM_conmat_T_group_diff_Ctrl_Scz_covariate_effect_%j.txt
#SBATCH --array=1-3
...
filename="$(find $DIR/file_ls/Ctrl_Scz/* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/file_ls/PALM_group_diff_Ctrl_Scz_covariate_effect_con00/;s/.txt//')"
outdir="${DIR}/Results/$truncname"
desmat="${DIR}/con_design_matrix/groupdiff_Ctrl_Scz_design_covariate_effect.csv"
conmat="${DIR}/con_design_matrix/contrast_matrix_covariates_effect.csv"
...
surfL=${HCP_DATA}/NEUR/ciftify/${exampleSubid}/MNINonLinear/fsaverage_LR32k/${exampleSubid}.L.midthickness.32k_fs_LR.surf.gii
surfR=${HCP_DATA}/NEUR/ciftify/${exampleSubid}/MNINonLinear/fsaverage_LR32k/${exampleSubid}.R.midthickness.32k_fs_LR.surf.gii
...
meansurface() {
    ...
    while read subids; do
	  dir=${HCP_DATA}/VA_files/Ctrl_Scz
...
runpalm() {
    ...
    palm -i ${fname}_sub.nii  -d $desmat -t $conmat -o results_sub -T -logp -n 1000 -precision "double"
